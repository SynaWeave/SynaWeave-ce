...
def runtime_dir() -> Path:
    override = os.environ.get("SW_RUN_DIR", "").strip() or os.environ.get(
        "SYNAWAVE_RUNTIME_DIR", ""
    ).strip()
...
def db_path() -> Path:
    override = os.environ.get("DB_PATH", "").strip() or os.environ.get(
        "SYNAWAVE_RUNTIME_DB_PATH", ""
    ).strip()
