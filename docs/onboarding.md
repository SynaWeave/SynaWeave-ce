...
After `bun run hooks:install`, the repo-owned hooks handle the normal environment-sync path for dependency-changing checkout, merge, rewrite, and push flows. Use `bun run sync` only when a hook reports a real sync failure and you need to rerun the sync command directly.
